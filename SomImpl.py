...
att = np.loadtxt("../Fluxo_experiments/GLCM/EXP_01/FEATURES_M1_CM8b.txt", usecols=[x for x in range(16)], delimiter=",")
...
for n in range(50):
    ...
    oData.random_training_test_by_percent([82, 133], use_train)
    ...
    for i in range(som.neurons_matrix.shape[0]):
        for j in range(som.neurons_matrix.shape[1]):
            ...
            som.neurons_labels[i, j, 0] = np.argmax(count_label)
...
exp.save("Experiment_02.gzip")



k_nn = 1
exp = Experiment.load("Experiment_02.gzip")
